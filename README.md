...
**Aurora has no Microsoft application registration yet.** Until a real client ID is provided to the build (`AURORA_MICROSOFT_CLIENT_ID`) and Microsoft approves the registration for Minecraft Services, Sign in reports `auth_configuration_missing` and Play reports an authentication blocker. The launch pipeline is implemented and tested against current official metadata with synthetic sessions and harmless child processes, but this is not yet a real end-user authenticated Minecraft launch.
