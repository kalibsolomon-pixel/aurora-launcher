...
Navigation separates global launcher destinations (Home, Instances, Accounts, Settings, About) from a contextual instance workspace: opening an instance from the sidebar shortcuts or the Instances page shows a stable header (breadcrumb, readiness badge, Open folder, Play) with Overview, Mods, and Settings tabs. Overview summarizes readiness, the active configuration, and the installed release; Settings edits that instance's configuration with an explicit save step; Mods inspects the actual managed instance `mods` directory without executing JARs, identifies the manifest-owned required Aurora Client artifact, reads bounded Fabric metadata, searches/filters locally, and safely enables, disables, or explicitly removes ordinary user-managed JARs. Refresh reconciles manual filesystem changes. Online discovery, installation, updates, and automatic dependency resolution through Modrinth, CurseForge, or any other provider are **not implemented**; Resource Packs, Shaders, and Logs also remain future workspace tabs.
