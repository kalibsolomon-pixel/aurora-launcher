...
This repository contains the launcher foundation through Phase 9 plus local instance content management: Tauri/Svelte desktop boundaries and versioned state; verified SHA-256, official SHA-1, and transport-observed artifact stores; official Minecraft and Fabric planning; safe staged game installation; persistent isolated Aurora instances; launcher-managed Java runtime provisioning; Microsoft → Xbox → Minecraft authentication; native launch assembly/process supervision; and filesystem-authoritative local mod inventory with safe enable, disable, and removal. Signing in uses the system browser with public-client OAuth (authorization code + PKCE), verifies Minecraft ownership and profile, stores only the Microsoft refresh credential in the operating system's credential store (the real Windows Credential Manager; Linux/macOS persistence is not implemented yet and fails deliberately), and restores Rust-only sessions with refresh-token rotation.
...
Navigation separates global launcher destinations (Home, Instances, Accounts, Settings, About) from a contextual instance workspace: opening an instance shows Overview, Mods, Resource Packs, Shaders, and Settings tabs. Mods inspects the isolated `mods` directory without executing JARs, protects required Aurora Client and Fabric API files, reads bounded Fabric metadata, and safely manages local JARs. Resource Packs and Shaders show local ZIP and folder packs with search, warnings, and explicit Refresh; regular ZIP files can be removed after confirmation, while folder packs and unsafe entries are left untouched. Each content tab also offers Modrinth Browse, which loads compatible projects and their icons immediately. Results offer Details for version and dependency review, plus Quick Install for the latest compatible version. Rust resolves required dependencies, verifies Modrinth's published SHA-512 and size, activates files safely, and records durable provider ownership. Resource packs are installed without activating them in Minecraft; shaders may require a separate shader loader. Installed provider content shows explicit dependency ownership and supports user-approved compatible update and removal previews. Existing content works offline. Automatic updates and CurseForge are not implemented; Logs remains a future workspace tab.
