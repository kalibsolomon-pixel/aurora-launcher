...
`ProviderInstallPlan` is Rust-only normalized input for a future adapter: validated `ArtifactSource` with expected SHA-256 and optional size plus a provider record. The backend-only transaction uses the shared verified cache, copies from the verified SHA-256 store to a private sibling, re-verifies the copy, activates via a no-overwrite hard link, then writes content state. Existing files and state records collide rather than being replaced; idempotent existing installs and update replacement are deferred. A failed state save removes the activated file when possible and reports failure; a crash between activation and state commit leaves an unclaimed local file that reconciliation will never call provider-managed. Provider removal renames the exact file aside, commits state without its record, restores it if that commit fails, then removes the temporary file. An interrupted removal may leave a hidden orphan for manual inspection; no broad cleanup runs. Mod, resource-pack, and shader-pack operations share one in-process instance lock. Cross-process transactions remain unsupported.

The frontend asks Rust for a registered instance plus content type and an opaque entry ID; it never supplies a path, download URL, or destination. `get_instance_content_context` derives the instance's Minecraft, Fabric Loader, Aurora version, and client environment from the pinned registry record for future provider filtering. A future Modrinth or CurseForge adapter will normalize provider metadata and dependencies into `ProviderInstallPlan`, while provider API DTOs, search, update discovery, and dependency solving stay outside this core. No provider command or API call ships in this phase.
