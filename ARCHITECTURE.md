...
└── InstanceWorkspace: { instanceId, tab ∈ { overview, settings } }
```

- The `developer` destination is stripped from production bundles together with its page, exactly as before.
- Instance-local tabs extend `InstanceTab`, never the global destinations: Mods, Resource Packs, Shaders, and Logs are future *instance* tabs (documented extension points, deliberately not built), and no global destination exists for them.
...
2. **Instance-local tabs** — Overview and Settings today, as a compact horizontal `tablist` (keyboard-operable, arrow keys included) in the main content region. An unsaved Settings draft marks the Settings tab with a text "Unsaved" marker.
...
- **Instance workspace** — the contextual shell for one instance: header (breadcrumb, identity, readiness badge, Open folder, Play), Overview and Settings tabs, and the future extension point for instance-content tabs. The Overview shows readiness (shared rows with Home), the active configuration, and the installed release pin; the Settings tab is the desired-configuration editor (General / Performance / Java / Display) with atomic Save changes and a deliberate Install new configuration action, plus rename and the managed-Java lifecycle context.
