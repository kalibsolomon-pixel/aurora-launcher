...
The shell is one persistent sidebar (Home, Instances, Accounts, Settings, About — plus Developer in development builds only, stripped from the production bundle together with its page) and one scrolling content region; the visual contract is `LAUNCHER_DESIGN_LANGUAGE.md`. Each screen presents only real backend state:
...
- **Instances** — creation from the essentials (name, Minecraft version from official metadata, loader policy), a list of instances with concise readiness, and a per-instance detail editor (General / Performance / Java / Display / Advanced) over the desired configuration with atomic Save changes and a deliberate Install new configuration action for install-affecting changes, plus selection, rename, retry, on-demand deep validation, and the managed Java lifecycle for the selected instance. Opaque instance ids, raw readiness dumps, and a second Play button are deliberately absent; mod management is not implemented and not shown.
...
- **Settings** — launcher-wide preferences. Today that is appearance only (theme and accent, applied live and persisted through the launcher configuration; see "Launcher appearance customization"). The page stays sparse and purposeful rather than inventing settings.
