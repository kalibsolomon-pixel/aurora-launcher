...
`instance_content` owns the closed `ContentType` vocabulary (`mod`, `resourcePack`, `shaderPack`), the corresponding `mods/`, `resourcepacks/`, and `shaderpacks/` directories, canonical containment checks, direct-child file validation, shared per-instance mutation exclusion, and a dedicated `instances/<id>/content-managed.json` document. The instance root is the launcher's game directory, so the resource-pack and shader-pack conventions resolve inside the isolated instance. A missing resource or shader directory inventories as empty; opening it creates only that derived directory. Symlinks, junctions, and reparse points are visible but never followed or mutated. Directory-form packs are visible and inspectable at the root but intentionally cannot be removed recursively. ZIP packs can be removed after opaque scan-token revalidation and explicit confirmation. Resource-pack metadata reads only root `pack.mcmeta` with a 256 KiB limit; ZIP inspection is capped at 4,096 entries and 512 MiB archive size. Shader inventory does not claim loader compatibility or universal metadata.

The content state schema is version 1. It records opaque provider/project/version/file IDs, the installed filename and SHA-256, content type, optional display version, Minecraft versions, loader and environment assertions, and required/optional/incompatible provider dependency references. IDs have no provider-specific interpretation. Serialization sorts entries by domain and filename; writes use a sibling temporary file and rename. Missing state means an empty provider inventory. Malformed, duplicate, or future-version state fails deliberately and is never repaired or overwritten. A missing recorded file is reported as stale provenance. A hash mismatch downgrades the visible entry to `unknown`, with actions blocked; a file without a record is never inferred to be provider-managed from its name, Fabric mod ID, or an online match. Required Aurora/Fabric API files take precedence over provider state.

`ProviderInstallPlan` is Rust-only normalized input from the Modrinth adapter: provider identity, compatibility, dependency references, a validated filename, and an expected-digest source. Modrinth publishes SHA-512 and SHA-1 for files, so the generic acquisition layer verifies the stronger published SHA-512 and exact size while streaming, then computes a local SHA-256 for the verified store and durable installed identity. A small SHA-512 → SHA-256 cache index is only a hint: cache hits are re-hashed against both values and size. SHA-1 is never mislabeled as SHA-256. Up to eight independent dependency artifacts are acquired concurrently through the pooled native HTTP client. Under one instance lock the transaction checks all collisions before mutation, copies each verified store object to a private sibling, re-verifies it, activates via a no-overwrite hard link, and writes the single content-state document last. Failure removes only names created by that transaction. Existing files and state records collide rather than being replaced; already installed exact provider versions are retained. A crash between activation and state commit can leave an unclaimed local file, which reconciliation will never call provider-managed. Provider removal renames the exact file aside, commits state without its record, restores it if that commit fails, then removes the temporary file. Dependencies remain installed when a parent is removed, since no safe reference-ownership model exists. Mod, resource-pack, and shader-pack operations share one in-process instance lock. Cross-process transactions and updates remain unsupported.
...
Browse and preview failures leave existing installed content and game launch untouched. Installed Modrinth files carry `provider=modrinth`, opaque project/version IDs, the file's published SHA-512 as its opaque file identity, the locally computed installed SHA-256, and dependency assertions. This is enough to identify a future update candidate, but safe replacement and dependency ownership are deferred; updates never occur automatically.

Quick Install accepts only the registered instance, content type, and project ID. Rust chooses the same release-preferred compatible version as Details, resolves its required dependency graph, fingerprints the normalized plan, re-resolves that exact version against current instance context and provider state, and refuses a changed fingerprint before using the same verified acquisition and activation transaction as the detailed path. An existing Modrinth project is left installed without an implicit update. No compatible version reaches artifact acquisition or filesystem mutation. The frontend shows a brief non-modal notification for that result and other provider errors.
